...
const TimelineCard: React.FC<TimelineCardProps> = ({ timelineEvent, i }) => {
    ...
    const getEventDescription = (eventName: string): string => {
        const descriptions: Record<string, string> = {
            "Guest Arrival": "We highly encourage you to arrive by 2:00 PM. Please be seated before the ceremony begins.",
            "Entourage & Wedding Ceremony": "Witness our vows and the beginning of our journey together as we exchange rings and promises at St. Therese Lahug.",
            "Refreshment & Photobooth": "Capture beautiful moments with the newlyweds and enjoy refreshments. Create lasting memories at our photobooth.",
            "Dinner & Heartfelt Toast": "Join us for a delightful dinner and heartfelt toasts as we celebrate our union at Beverly View Hills Pavilion.",
            "A Night to Remember": "Dance, celebrate, and make unforgettable memories with us as the night unfolds."
        };
        ...
        >
            ...
            <div style={container}>
                {timelineEvents.map((timelineEvent, i) => (
                    <TimelineCard i={i} timelineEvent={timelineEvent} key={`${timelineEvent.time}-${i}`} />
                ))}
            </div>
            {/* Bottom Decorative */}
            <motion.div
